...
abstract class ModelTestCase extends TestCase
{
     ...
     *
     * - `getQuery()`: assert query has not been modified or modified properly.
     * - `getForeignKey()`: any `HasOneOrMany` or `BelongsTo` relation, but key type differs.
     * - `getQualifiedParentKeyName()`: in case of `HasOneOrMany` relation, there is no `getLocalKey()` method,
     *    so this one should be asserted.
     */
    protected function assertHasOneRelation(
        ...
        $this->assertEquals($model->getTable() . '.' . $parent, $relation->getQualifiedParentKeyName());
        $this->assertEquals($related->getTable() . '.' . $key, $relation->getQualifiedForeignKeyName());
    }
    ...
    protected function assertBelongsToManyRelation(
        ...
        $felatedPivotKey = $relation->getQualifiedRelatedPivotKeyName();

        $foreignPivotKey = substr($foreignPivotKey, strpos($foreignPivotKey, ".") + 1);
        $felatedPivotKey = substr($felatedPivotKey, strpos($felatedPivotKey, ".") + 1);

        $this->assertEquals($related->getForeignKey(), $felatedPivotKey);
        $this->assertEquals($model->getForeignKey(), $foreignPivotKey);
    }
}
